...
int execute_path(char *ecmd, char **argv)
{
    ...
    if (args == NULL)
    {
        free(ecmd); /* Moved from the original position */
        return 0;
    }
    ...
    {
        free(args);
        return 0; /* Calling built-in functions if path is NULL */
    }
    else if (pathcmd != NULL)
    {
        ...
        /* If execmd is successful, it won't reach here, so we can free immediately */
        free(args);
        free(pathcmd);
    }
    else
